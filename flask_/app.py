...
def search():
    try:
        ...
        if not db["quotes"].create_index([("quote", "text"), ("author", "text")]):
            # Indexer la collection
            db[collection_name].create_index([("quote", "text"), ("author", "text")])
...
def random():
    try:
        ...
        results = db.quotes.aggregate([{"$sample": {"size": quote_number}}]) # Selectionne une citation au hasard, $sample permet de selectionner une citation au hasard
        liste = list(results)
        new_liste = []
        for result in liste:
            new_liste.append({"quote": result["quote"], "author": result["author"]})
